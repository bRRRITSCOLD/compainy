...
**Composing pages and templates from the component library.** When the component library is ready and the UX designer has defined page/template structures in Figma, invoke this agent to scaffold Next.js routes and layout templates. It keeps pages thin (data-fetching coordinators only) and pushes domain logic to backend services.
...
- **`pages-templates`** — composes components into Next.js layout templates and page routes following the UX page/template structure; keeps pages thin and delegates domain logic to backend services.
